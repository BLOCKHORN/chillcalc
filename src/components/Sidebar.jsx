...
import { 
  LayoutDashboard, Wallet, ArrowLeftRight, Target, LogOut, Users, 
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed top-0 left-0 w-64 h-screen bg-surface-solid border-r border-border-subtle flex flex-col z-50">
      ...
      <div className="flex items-center gap-3 px-6 py-10">
        ...
        <span className="font-bold text-lg tracking-tighter text-text-main uppercase">
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isActive 
                  ? "text-text-main bg-text-main/[0.04]" 
                  : "text-text-muted hover:text-text-main hover:bg-text-main/[0.02]"
              )}
            >
              <Icon 
                size={16} 
                ...
                className={clsx(
                  ...
                  isActive ? "text-brand-emerald" : "text-text-muted group-hover:text-text-main"
                )} 
              />
              ...
              {isActive && (
                <motion.div 
                  ...
                  className="absolute inset-0 bg-text-main/[0.01] rounded-xl border border-text-main/5"
      ...
      {/* Footer Actions */}
      <div className="p-4 bg-text-main/[0.005] border-t border-border-subtle">
        
        <div className="flex flex-col gap-1 mb-2">
          <button 
            onClick={toggleTema}
            className="flex items-center justify-between w-full px-4 py-2.5 rounded-xl text-[12px] font-bold text-text-muted hover:text-text-main transition-all group"
          >
            <div className="flex items-center gap-3">
               {tema === 'dark' ? <Moon size={15} /> : <Sun size={15} className="text-warning" />}
               <span>Tema {tema === 'dark' ? 'Oscuro' : 'Claro'}</span>
            </div>
            <div className={clsx(
              "w-6 h-3 rounded-full border border-border-subtle relative transition-colors", 
              tema === 'light' && "bg-warning/20 border-warning/40"
            )}>
               <div className={clsx(
                 "absolute top-0.5 w-1.5 h-1.5 rounded-full transition-all", 
                 tema === 'light' ? "right-0.5 bg-warning" : "left-0.5 bg-text-muted"
               )} />
            </div>
          </button>
        ...
        <div className="px-4 py-3 rounded-xl bg-text-main/[0.02] border border-border-subtle flex items-center gap-3">
           ...
           <div className="flex-1 overflow-hidden">
              <p className="text-[12px] font-bold text-text-main truncate">{email || 'Invitado'}</p>
